...
export function evaluateBalance(dates: string[], transactions: Transaction[], opts?: { quotes?: Quote[] }): Balance[] {
  ...
  while (true) {
    ...
    const nextTransaction = nextTransactionIndex < transactions.length ? transactions[nextTransactionIndex] : undefined
...
export function updateBalanceByTransaction(b: Balance, transaction: Transaction): Balance {
  ...
  switch (transaction.data.type) {
    case 'cashDeposit':
      return updateBalanceByTransactionCashDeposit(b2, transaction, transaction.data)
    case 'cashWithdrawal':
      return updateBalanceByTransactionCashWithdrawal(b2, transaction, transaction.data)
    case 'cashTransfer':
...
function updateBalanceByTransactionCashDeposit(
  b: Balance,
  ...
  return iterate(
    ...
    (remainingAmount, b) => {
      const openCashPositionIndex = findIndexLeft(
        ...
        p => p.accountId === data.cashAccountId && p.amount.lt(0)
      )
      if (openCashPositionIndex >= 0) {
        const openCashPosition = b.cashPositions.open[openCashPositionIndex]
        const consumedAmount = BigNumber.min(openCashPosition.amount.negated(), remainingAmount).negated()
        ...
        if (!openCashPositionUpdated.amount.eq(0)) {
          return [
            remainingAmount.plus(consumedAmount),
            {
              ...b,
              cashPositions: {
                open: [
...
function updateBalanceByTransactionCashWithdrawal(
  b: Balance,
  transaction: Transaction,
  ...
  return iterate(
    ...
    (remainingAmount, b) => {
      const openCashPositionIndex = findIndexLeft(
        b.cashPositions.open,
        p => p.accountId === data.cashAccountId && p.amount.gt(0)
      )
      if (openCashPositionIndex >= 0) {
        const openCashPosition = b.cashPositions.open[openCashPositionIndex]
        const consumedAmount = BigNumber.min(openCashPosition.amount, remainingAmount)
        const consumedOpenPrice = BigNumber(openCashPosition.openPrice)
          .multipliedBy(consumedAmount)
          .dividedBy(openCashPosition.amount)
        const openCashPositionUpdated: OpenCashPosition = {
            ...
            cashPositions: {
              ...
              open: [
                ...b.cashPositions.open,
                {
                  ...
                  state: 'open',
                  accountId: data.cashAccountId,
                  amount: remainingAmount.negated(),
                  openTransactionId: transaction.id,
                  openDate: transaction.date,
                  openTime: transaction.time,
                  openPrice: remainingAmount.negated(),
                },
              ],
            },
          },
...
function updateBalanceByTransactionCashTransfer(
  ...
  const b2 = updateBalanceByTransactionCashWithdrawal(b, transaction, {
    type: 'cashWithdrawal',
    cashAccountId: data.fromCashAccountId,
    cashAmount: data.feeCashAmount,
  })
  const b3 = updateBalanceByTransactionCashWithdrawal(b2, transaction, {
    type: 'cashWithdrawal',
    cashAccountId: data.fromCashAccountId,
...
function updateBalanceByTransactionAssetSell(
  ...
  const b3 = updateBalanceByTransactionCashDeposit(b2, transaction, {
    type: 'cashDeposit',
    cashAccountId: data.cashAccountId,
...
function updateBalanceByTransactionFee(
  ...
  return updateBalanceByTransactionCashWithdrawal(b, transaction, {
    type: 'cashWithdrawal',
    cashAccountId: data.cashAccountId,
    cashAmount: data.feeCashAmount,
  })
...
function updateBalanceByTransactionDividend(
  ...
  // TODO must be considered 0 price
  return updateBalanceByTransactionCashDeposit(b, transaction, {
    type: 'cashDeposit',
    cashAccountId: data.cashAccountId,
    cashAmount: data.cashAmount,
  })

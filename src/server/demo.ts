...
export async function generateDemoPortfolio(database: Database, userId: string): Promise<Portfolio> {
  ...
  await database.classifications.assignToClassification(riskClassificationNoRisk.id, accountSavings.id, null)
  await database.classifications.assignToClassification(riskClassificationModerateRisk.id, accountStocks.id, null)
  await database.classifications.assignToClassification(riskClassificationHighRisk.id, accountCrypto.id, null)
  await database.classifications.assignToClassification(riskClassificationModerateRisk.id, null, msciWorldAsset.id)
  return portfolio
}

...
export function createRpcV1Attachment(database: Database) {
  return {
    listAttachments: createRpcCall(
      ...
      z.array(attachmentSchema.extend({ transactionCount: z.number() })),
    ...
    retrieveAttachment: createRpcCall(byIdSchema, attachmentSchema, async (ctx: RpcCtx, input) => {
      if (!ctx.user) throw RpcError.unauthorized()
    ...
    deleteAttachment: createRpcCall(byIdSchema, z.void(), async (ctx: RpcCtx, input) => {
      if (!ctx.user) throw RpcError.unauthorized()
      const attachment = await database.attachments.find(input.id)
      if (!attachment || attachment.userId !== ctx.user.id) throw RpcError.badRequest(`Unknown attachment ${input.id}`)
      await database.attachments.delete(input.id)
    }),
    uploadAttachment: createRpcCall(fileSchema, attachmentSchema, async (ctx: RpcCtx, input) => {
      if (!ctx.user) throw RpcError.unauthorized()
    ...
    downloadAttachment: createRpcCall(byIdSchema, fileSchema, async (ctx: RpcCtx, input) => {
      if (!ctx.user) throw RpcError.unauthorized()
      const attachment = await database.attachments.find(input.id)
      if (!attachment || attachment.userId !== ctx.user.id) throw RpcError.badRequest(`Unknown attachment ${input.id}`)
      const bytes = await database.attachments.read(input.id)
      return { fileName: attachment.fileName, dataUrl: renderDataUrl(attachment.mimeType, bytes.toString('base64')) }
    }),
    downloadAttachmentAsPng: createRpcCall(byIdSchema, z.array(z.string()), async (ctx: RpcCtx, input) => {
      if (!ctx.user) throw RpcError.unauthorized()
      const attachment = await database.attachments.find(input.id)
      if (!attachment || attachment.userId !== ctx.user.id) throw RpcError.badRequest(`Unknown attachment ${input.id}`)
      ...
      return pngsBytes.map(pngBytes => renderDataUrl('image/png', pngBytes.toString('base64')))
    }),
    retrieveAttachmentContent: createRpcCall(
      byIdSchema,
      attachmentContentSchema.nullable(),
      async (ctx: RpcCtx, input) => {
        if (!ctx.user) throw RpcError.unauthorized()
        const attachment = await database.attachments.find(input.id)

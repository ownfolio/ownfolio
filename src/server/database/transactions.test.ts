...
it(
  ...
  databaseTest(async db => {
    ...
    await expect(db.transactions.listByUserId(u.id)).resolves.toEqual([t2, t1])
    await expect(db.transactions.listByUserId('user_???')).resolves.toEqual([])
    await expect(db.transactions.listByUserId(u.id, { type: 'cashDeposit' })).resolves.toEqual([t1])
    await expect(db.transactions.listByUserId(u.id, { type: 'assetBuy' })).resolves.toEqual([t2])
    await expect(db.transactions.listByUserId(u.id, { fromDate: '2020-01-02' })).resolves.toEqual([t2])
    await expect(db.transactions.listByUserId(u.id, { toDate: '2020-01-01' })).resolves.toEqual([t1])
    await expect(db.transactions.listByUserId(u.id, { portfolioId: p1.id })).resolves.toEqual([t2, t1])
    ...
    await expect(db.transactions.listByUserId(u.id, { accountId: a1.id })).resolves.toEqual([t2, t1])
    await expect(db.transactions.listByUserId(u.id, { accountId: a2.id })).resolves.toEqual([t2])
    await expect(db.transactions.listByUserId(u.id, { accountId: 'act_???' })).resolves.toEqual([])
    await expect(db.transactions.listByUserId(u.id, { assetId: s1.id })).resolves.toEqual([t2])
    await expect(db.transactions.listByUserId(u.id, { assetId: 'ast_???' })).resolves.toEqual([])
    await expect(db.transactions.listByUserId(u.id, { reference: 'ref1' })).resolves.toEqual([t1])
    await expect(db.transactions.listByUserId(u.id, { reference: 'ref2' })).resolves.toEqual([t2])

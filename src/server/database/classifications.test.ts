...
it(
  ...
  databaseTest(async db => {
    ...
    await expect(db.classifications.assignToClassification(c11.id, null, null)).rejects.toThrowError()
    await db.classifications.assignToClassification(c11.id, a1.id, null)
    await expect(db.classifications.listAssignmentsByUserId(u1.id)).resolves.toEqual([
      { classificationId: c11.id, accountId: a1.id, assetId: null },
    ])
    await expect(db.classifications.listAssignmentsByUserId(u2.id)).resolves.toEqual([])
    await db.classifications.assignToClassification(c21.id, a2.id, null)
    await expect(db.classifications.listAssignmentsByUserId(u1.id)).resolves.toEqual([
      { classificationId: c11.id, accountId: a1.id, assetId: null },
    ])
    await expect(db.classifications.listAssignmentsByUserId(u2.id)).resolves.toEqual([
      { classificationId: c21.id, accountId: a2.id, assetId: null },
    ])
  }),
  60000
)
